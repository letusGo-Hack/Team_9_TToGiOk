        ...
        NavigationStack(path: $path) {
            VStack(spacing: 20) {
                ...
                .onAppear {
                    ...
                    NotificationCenter.default.addObserver(forName: .navToResults, object: nil, queue: .main) { _ in
                        path = NavigationPath()
                        path.append(NavigationRouter.navToResultView)
                    }
                    
                    NotificationCenter.default.addObserver(forName: .navToShake, object: nil, queue: .main) { _ in
                        path = NavigationPath()
                        path.append(NavigationRouter.navToShakeView)
                    }
                }
            ...
            .background(.white)
            .navigationDestination(for: NavigationRouter.self) { nav in
                switch nav {
